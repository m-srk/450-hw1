...
double compute_mflops(int N, int R) {
    ...
    // triad benchmarking loop
    for (j=0; j<R; j++) {
        for (k=0; k<N; k++) {
            A[k] = B[k] + C[k] * D[k]; 
        }
        // dummy call to turnoff compiler optimization ->
        if (A[2] < 0) {
            dummy_func(A, B, C, D);
        }
    }
...
int main (int argc, char** argv) {
    ...
    if (argc > 3) {
        ...
        // TODO change this to default to the vals given in assign.
        // for now, running benchmarks for series of N, R
        // N ranges from 10 to 10 million
        N = 1000;
        R = 1000000;
        cout << compute_mflops(N, R) << endl;
    }
